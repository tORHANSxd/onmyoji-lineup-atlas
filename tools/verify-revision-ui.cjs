...
async function main(){
 ...
 const loginActions=[];local('ta-status',()=>status);local('ta-action',(action,params)=>{loginActions.push({action,params});if(action==='forget')publish({remembered_accounts:status.remembered_accounts.filter(a=>a.id!==params.account_id)});return status;});local('ta-logout',()=>{publish({authenticated:false,selected_avatar:''});return status;});
