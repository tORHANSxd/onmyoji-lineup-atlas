...
// YUQI_SNAPSHOT_REPORT can select a separate report without replacing release evidence.
...
for(const [i,file] of files.entries()){
 ...
 const heroes=Object.values(a.heroes),souls=Object.values(a.souls),rawSouls=raw.format==='yys-desktop-cache-v1'?raw.souls:raw.hero_equips;let bases=0;
 assert.equal(heroes.length,Object.keys(raw.heroes).length,'式神数量');assert.equal(souls.length,rawSouls.length,'御魂数量');
