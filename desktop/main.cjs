...
let win, official, taSession, queryQueue, loginEpoch=0, loggedIn=false, selectedRole='';
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('ta-action',async(e,action,params)=>{trusted(e);if(['resume','forget'].includes(action)){
  ...
  }else{if(!['init','qr','select','roles'].includes(action))throw new Error('不支持的登录操作');await taSession.request(action,params);}return taSession.status();});
  ...
  win.loadURL(pageURL);
  if(!isSmoke&&credentials.data.activeId)taSession.resume(credentials.data.activeId).catch(()=>{});
