...
function harness(){
 ...
 return {messages,results:()=>Object.fromEntries(messages.filter(m=>m.result).map(m=>[m.id,m.result])),send:data=>context.onmessage({data}),drain(){let n=100;while(timers.length&&n-->0)timers.shift()();assert.ok(n>0);}};
}
