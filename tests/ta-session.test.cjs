...
function harness(){
 ...
 const session=new TASession({helperPath:__filename,onStatus:s=>statuses.push(s),spawnProcess:(file,args,options)=>{
...
test('登录桥接拒绝任意操作、角色路径及损坏文字码',()=>{
 for(const [action,params] of [['shell',{}],['query',{code:'|TA|opaque-key'}],['query',{code:'|TA|'+'a'.repeat(32),token:'secret'}],['select',{server_id:'../private'}],['qr',{url:'https://other.invalid'}]])assert.throws(()=>validateAction(action,params));
 assert.doesNotThrow(()=>validateAction('query',{code:'|TA|'+'a'.repeat(32)}));
});
test('通过隐藏子进程传递请求，分片结果保持完整，登录不落盘',async()=>{
...
test('退出登录中止查询，迟到回包不能恢复旧账号，允许新会话',async()=>{
 const h=harness();const pending=h.session.request('query',{code:'|TA|'+'a'.repeat(32)}),old=h.children[0];h.session.stop();await assert.rejects(pending,/退出登录/);assert.equal(old.killed,true);assert.equal(h.session.status().authenticated,false);
