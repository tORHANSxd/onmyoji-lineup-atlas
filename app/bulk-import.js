...
function parse(text,existing=[]){
 ...
 for(let index=0;index<lines.length;index++){
  ...
  const row={line:index+1,code,title:title||'未命名阵容',dungeon:dungeon||'待分类',notes,status:'valid',reason:'可添加 · 内容待解析'};
  if(fields.length>4)row.reason='最多 4 列；名称、用途和备注中请勿使用分隔符 || 或制表符。';
  else if(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f\ufffd]/.test(raw))row.reason='含有无效字符；文件请另存为 UTF-8 文本。';
  else if(kind==='pipe-ta'&&!/^\|TA\|[a-fA-F0-9]{32}$/.test(code))row.reason='文字码需以 |TA| 开头，后接完整的 32 位分享键。';
  ...
  else{seen.set(code,row.line);result.entries.push({code,title:row.title,dungeon:row.dungeon,notes});}
